...
- GitHub Actions run `29391706307` cleared the complete Ubuntu test, type-check, build, package-inspection, credential, and Marketplace publish sequence for `0.1.4`; `vsce publish` reported success. Its final verifier failed only because the Marketplace API had not exposed `0.1.4` during the original six checks (about 50 seconds). The public API exposed the manually submitted `0.1.1` with exact local SHA-256 `12a32b852c384bd733aadefe4be44eba2d061b005b4e4ca9f97845bc4dbc427d`, then exposed automated `0.1.4` at `2026-07-15T05:39:00.043Z` with SHA-256 `ce59b53e6180cf94632c5278d7ce3dfd2ac534d3283aa80e9a89b8911cbcb77b`, roughly six minutes after the publish command completed. The verifier now prints its expected SHA and performs 60 checks at ten-second intervals so normal Marketplace propagation does not turn an accepted publish into a false-negative workflow result.
- GitHub Actions run `29392197803` cleared tests through credential validation for `0.1.5`, then `vsce 3.9.2` returned `Request timeout` after its three-minute unsigned-upload request. The ordinary unsigned publish path does not use the retry policy that `vsce` applies to signed-package publishing. The publish step now retries that exact package once with `--skip-duplicate`; a second timeout is treated as an ambiguous server outcome and handed to the existing exact-version/exact-SHA verifier. Any non-timeout publish error still fails immediately, and the job timeout is 25 minutes to cover the bounded retry plus propagation verification.
